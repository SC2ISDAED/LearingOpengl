...
public:
	...
	Shader(const GLchar * vertexPath, const GLchar * fragmentPath,const GLchar * geometryPath=nullptr)
	{
		...
		try
		{
			...
			std::stringstream vShaderStream, fShaderStream,geometryShaderSteam;
			//read file to stream
			...
			{
				std::cout << "ERROR::FRAGMENTCODE OR VERTEXCODE IS NULL";
			}
			if (geometryPath!=nullptr)
			{
				geometryFile.open(geometryPath);
				geometryShaderSteam << geometryFile.rdbuf();
				geometryFile.close();
				geometryCode = geometryShaderSteam.str();
			}

		}
		catch (std::ifstream::failure e)
		{
			std::cout << "ENRROR::SHADER::NOT::SUCCESFULLY_READ" << std::endl;		
		}
		...
		if (geometryPath!=nullptr)
		{
			const GLchar * gometryShaderCode = geometryCode.c_str();
			gometry = glCreateShader(GL_GEOMETRY_SHADER);
			glShaderSource(gometry, 1, &gometryShaderCode, NULL);
			glCompileShader(gometry);
			checkCompileErrors(gometry, "GEOMETRY");
		}
		this->Program = glCreateProgram();
		glAttachShader(this->Program, vertex);
		glAttachShader(this->Program, fragment);
		if (geometryPath != nullptr)
			glAttachShader(this->Program, gometry);
		glLinkProgram(this->Program);
		checkCompileErrors(this->Program, "PROGRAM");
		glDeleteShader(vertex);
		glDeleteShader(fragment);
		if (geometryCode != "")
			glDeleteShader(gometry);
	}
	void Use()
	{
